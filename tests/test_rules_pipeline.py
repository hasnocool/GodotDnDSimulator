...
@pytest.mark.asyncio
async def test_build_rejects_direct_artifact_policy_mismatch(tmp_path: Path) -> None:
    artifact = _fixture_artifact(tmp_path)
    official = SourceRegistry.from_path(ROOT / "config/rules/sources.json").require(
        "wotc-srd-5.2.1-en"
    )
    ...
    with pytest.raises(SourcePolicyError, match="requested URL"):
        await build_from_artifact(
            artifact=replace(artifact, requested_url="https://media.dndbeyond.com/other.pdf"),
            **kwargs,
        )
    with pytest.raises(SourcePolicyError, match="media type"):
        await build_from_artifact(
            artifact=replace(artifact, media_type="text/html"),
            **kwargs,
        )
    with pytest.raises(SourcePolicyError, match="unapproved host"):
        await build_from_artifact(
            artifact=replace(artifact, final_url="https://example.com/source.pdf"),
            **kwargs,
        )
