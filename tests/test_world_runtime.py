...
from godot_dnd_engine.world import (
    QuestStatus,
    WorldRuntime,
    demo_campaign,
    replay_world_events,
...
def test_world_start_travel_and_actions_are_authoritative() -> None:
    ...
    _command(
        runtime,
        "world.start",
        {"party_ids": ["actor:hero-a", "actor:hero-b"]},
    )
...
def test_shop_inventory_equipment_and_currency_flow() -> None:
    ...
    assert (
        runtime.state.equipped_map()["actor:hero-a|slot:utility"]
        == "item:rope-coil"
    )
    _command(
        runtime,
...
def test_world_snapshot_restore_preserves_state_and_future_rng() -> None:
    runtime = _runtime(seed=123)
    _command(runtime, "world.start", {"party_ids": ["actor:hero-a"]})
    _command(runtime, "world.travel", {"area_id": "area:old-road"})
    _command(
        runtime,
    ...
    restored = restore_world_runtime(demo_campaign(), snapshot)
    assert restored.state == runtime.state
    assert restored.rng.snapshot() == runtime.rng.snapshot()
    assert restored.events == runtime.events
