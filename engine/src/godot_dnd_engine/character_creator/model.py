...
class InventoryGrant:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.quantity, bool) or not isinstance(self.quantity, int) or self.quantity < 1:
            raise ValidationError("creator inventory grant quantity must be an integer >= 1")
        if self.equip_slot is not None and not self.equip_slot.strip():
            raise ValidationError("creator inventory equip_slot must be None or non-empty")
...
class CreationChoice:
    ...
    def __post_init__(self) -> None:
        if not self.choice_id.strip() or not self.name.strip():
        ...
        if len(self.skill_proficiencies) != len(set(self.skill_proficiencies)):
            raise ValidationError("creator choice skill proficiencies must be unique")
        if len(self.save_proficiencies) != len(set(self.save_proficiencies)):
            raise ValidationError("creator choice save proficiencies must be unique")
        if len(self.training_proficiencies) != len(set(self.training_proficiencies)):
            raise ValidationError("creator choice training proficiencies must be unique")
        if len(self.spell_ids) != len(set(self.spell_ids)) or len(self.feature_ids) != len(set(self.feature_ids)):
        ...
        if len(bonus_abilities) != len(set(bonus_abilities)):
            raise ValidationError("creator choice ability bonuses must be unique by ability")
        ...
        for label, value in (
            ...
            if value is not None and (isinstance(value, bool) or not isinstance(value, int) or value < 0):
                raise ValidationError(f"creator {label} must be None or an integer >= 0")
        if isinstance(self.unlock_level, bool) or not isinstance(self.unlock_level, int) or not 1 <= self.unlock_level <= 20:
            raise ValidationError("creator unlock_level must be an integer from 1 through 20")
...
class CreationGroup:
    ...
    def __post_init__(self) -> None:
        if not self.group_id.strip() or not self.choice_ids:
            raise ValidationError("creator groups require a non-empty ID and choices")
        if len(self.choice_ids) != len(set(self.choice_ids)):
            raise ValidationError("creator group choice IDs must be unique")
        if any(not item.strip() for item in self.choice_ids):
...
class AbilityScorePolicy:
    ...
    def __post_init__(self) -> None:
        if not self.method_id.strip() or len(self.values) != len(Ability):
            raise ValidationError("ability policy requires an ID and exactly six values")
        if any(isinstance(value, bool) or not isinstance(value, int) or not 1 <= value <= 30 for value in self.values):
            raise ValidationError("ability policy values must be integer scores from 1 through 30")
...
class CharacterCreatorCatalog:
    ...
    def __post_init__(self) -> None:
        ...
        for group in self.groups:
            ...
            if missing:
                raise ValidationError(f"creator group {group.group_id!r} references unknown choices")
        for choice in self.choices:
            if (choice.requires | choice.conflicts) - known:
                raise ValidationError(f"creator choice {choice.choice_id!r} references unknown choices")
        policy_ids = [item.method_id for item in self.ability_policies]
        if len(policy_ids) != len(set(policy_ids)) or not policy_ids:
            raise ValidationError("creator catalog requires unique ability policies")
...
class CharacterDraft:
    ...
    def __post_init__(self) -> None:
        if not self.actor_id.strip() or not self.name.strip() or not self.ability_method_id.strip():
            raise ValidationError("character draft actor/name/ability method must be non-empty")
        ...
        if set(ability_ids) != set(Ability) or len(ability_ids) != len(set(ability_ids)):
            raise ValidationError("character draft must assign each ability exactly once")
        for _, score in self.base_ability_scores:
            if isinstance(score, bool) or not isinstance(score, int) or not 1 <= score <= 30:
                raise ValidationError("character draft ability scores must be integer values from 1 through 30")
        appearance_keys = [key for key, _ in self.appearance]
        if len(appearance_keys) != len(set(appearance_keys)) or any(not key.strip() for key in appearance_keys):
            raise ValidationError("character appearance keys must be unique non-empty strings")
...
class CharacterRecord:
    ...
    def __post_init__(self) -> None:
        for value in (self.catalog_id, self.species_id, self.background_id, self.class_id, self.ability_method_id):
            if not value.strip():
                raise ValidationError("character record identity fields must be non-empty")
        if len(self.spell_ids) != len(set(self.spell_ids)) or len(self.feature_ids) != len(set(self.feature_ids)):
            raise ValidationError("character record spell/feature IDs must be unique")
