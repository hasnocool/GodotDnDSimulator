...
def _cached_artifact_is_valid(artifact: SourceArtifact, source_path: Path) -> bool:
...
async def fetch_source(
    ...
    *,
    ...
    if cached is not None and not force:
        valid = await asyncio.to_thread(_cached_artifact_is_valid, cached, source_path)
        if valid and (policy.expected_sha256 is None or cached.sha256 == policy.expected_sha256):
            return cached
